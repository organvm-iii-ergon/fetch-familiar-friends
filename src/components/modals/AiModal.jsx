...
const AiModal = ({ isOpen, onClose, currentBreed = null }) => {
  ...
  const generateAiResponse = (userMessage) => {
    ...
    if (currentBreed) {
      ...
      if (lowerMessage.includes("train") || lowerMessage.includes("training")) {
        ...
        lowerMessage.includes("temperament") ||
        ...
        lowerMessage.includes("breed") ||
        lowerMessage.includes("about") ||
        lowerMessage.includes("info") ||
        lowerMessage.includes("this dog")
      ) {
    ...
    if (lowerMessage.includes("train") || lowerMessage.includes("training")) {
      ...
      if (currentBreed) {
        return (
          getBreedSpecificResponse(currentBreed, "info") ||
          "Let me know what you'd like to know about this breed!"
        );
      }
      ...
      if (
        currentBreed &&
        (lowerMessage.includes("this") || lowerMessage.includes("these"))
      ) {
        return (
          getBreedSpecificResponse(currentBreed, "info") ||
          `That's a great question! ${currentBreed}s are wonderful dogs. What specific aspect would you like to know more about?`
        );
      }
      return (
        "That's a great question! For personalized advice about " +
        userMessage +
        ", I recommend consulting with your veterinarian or a certified dog trainer. Is there anything else I can help you with regarding general dog care, training tips, or breed information?"
      );
    }
  };
  ...
  const handleSend = async () => {
    ...
    const userMessage = {
      role: "user",
      content: inputMessage.slice(0, 500)
    };
    ...
    setTimeout(
      () => {
        const aiResponse = {
          role: "assistant",
          content: generateAiResponse(inputMessage.slice(0, 500)),
        };
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="AI Assistant" size="xl">
      <div className="flex flex-col h-[500px]">
        ...
        <div className="flex flex-col gap-2">
          {inputError && (
            <div className="text-red-500 text-xs px-1">{inputError}</div>
          )}
        ...
        <p className="text-xs text-gray-500 mt-2 text-center">
          This is a demo AI assistant. For real medical advice, always consult a
          veterinarian.
        </p>
      </div>
    </Modal>
