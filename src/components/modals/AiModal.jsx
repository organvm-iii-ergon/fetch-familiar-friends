...
const AiModal = ({ isOpen, onClose, currentBreed = null }) => {
  ...
  const handleSend = async () => {
    ...
    // Security & Content Safety Checks
    if (!isFamilyFriendly(inputMessage)) {
      setError('Please keep the conversation family-friendly. Profanity is not allowed.');
      return;
    }
    ...
    const userMessage = {
      role: 'user',
      content: sanitizedMessage.slice(0, 500) // Ensure max length
    };
