...
export function validateBreed(breed, type = "dog") {
  ...
  if (type === "dog") {
    ...
    const mainBreedCheck = validateString(breed.mainBreed, "mainBreed", {
      minLength: 1,
    });
    if (!mainBreedCheck.valid) errors.push(mainBreedCheck.error);
  } else if (type === "cat") {
    const descCheck = validateString(breed.description || "", "description", {
      maxLength: 2000,
    });
    ...
    if (breed.traits) {
      ...
      for (const field of traitFields) {
        if (breed.traits[field] !== undefined) {
          const traitCheck = validateNumber(
            breed.traits[field],
            `traits.${field}`,
...
export function validateContentItem(item, type) {
  ...
  if (type === "fact" || type === "quote") {
    ...
    if (typeof item !== "string") {
      ...
      const lengthCheck = validateString(item, type, {
        minLength: 10,
        maxLength: 500,
      });
    ...
    if (typeof item !== "object" || item === null) {
      ...
      const emojiCheck = validateString(item.emoji, "emoji", {
        minLength: 1,
        maxLength: 10,
      });
      if (!emojiCheck.valid) errors.push(emojiCheck.error);

      const textCheck = validateString(item.text, "text", {
        minLength: 3,
        maxLength: 100,
      });
      if (!textCheck.valid) errors.push(textCheck.error);

      const descCheck = validateString(item.description, "description", {
        minLength: 5,
        maxLength: 200,
      });
      if (!descCheck.valid) errors.push(descCheck.error);
...
export function isFamilyFriendly(content) {
  ...
  for (const word of profanity) {
    // Escape special regex characters in the word if any
    const escapedWord = word.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    // Use word boundaries to avoid matching substrings like "class" or "hello"
